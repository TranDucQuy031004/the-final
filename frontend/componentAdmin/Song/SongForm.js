...
const SongForm = (props) => {
    ...
    const checkData = () => {
        let isValid = true;
        let txtTitle = false, txtArtist = false, image = false, mp3File = false;
    ...
    const calculateAudioDuration = (audio) => {
        if (audio != undefined) {
            let blob = URL.createObjectURL(audio);
            let audioFile = new Audio(blob);
            setAudio(audioFile);
        }
    };
    ...
    useEffect(() => {
        ...
        if (songCurrent != undefined) {
            setForm({
                ...form,
                txtArtist: transferDataArtist(songCurrent.artistSongs),
                txtTitle: songCurrent.title,
                timePlay: songCurrent.timePlay,
                image: songCurrent.image,
                mp3File: songCurrent.mediaUrl,
                ...
                <Grid container spacing={0}>
                    <div className="ms_pro_form">
                        ...
                        <div className="form-group">
                            ...
                            <span style={error.image ? { display: '', textAlign: 'center' } : { display: 'none' }} className="error-form">Hãy chọn ảnh</span>
                            <br />
